...
## 架构
一台机器运行一个 judge-system，并启动一些 runner，runner 可以执行不同的任务：
  * 编译测试：负责执行编译，编译完成后的文件允许各个 runner 评测时读取（通过 aufs 来只读挂载）
...
目前有以下的 check script
  ...
  3. static: 允许评测静态测试
  4. memory: 允许评测内存测试
  5. gtest: 允许评测单元测试，单元测试不需要测试数据的输入

其中编译测试比较特殊，使用 compile.sh 来完成工作。

check script 通过返回值来确定评分，比如返回 42 表示 AC，43 表示 WA。对于静态测试、内存测试等需要直接返回外部程序的测试结果的（比如 oclint、valgrind 的输出），将这些评测结果经过必要的转换后放到指定文件夹中供评测系统读取并直接返回给评测服务端。
