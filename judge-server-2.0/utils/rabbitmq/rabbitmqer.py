...
class RabbitMQer(object):
    def __init__(self, host, port, exchange, routing_key, timeout=50):
        ...
        self.channel.queue_declare(queue=routing_key, durable=True, exclusive=False)
    ...
    def connect(self):
        ...
        self.channel = self.conn.channel()
        self.channel.queue_declare(queue=self.routing_key, durable=True, exclusive=False)
...
submissionMQ = RabbitMQer(g_config.get("RabbitMQ", "hostname"),
                          ...
                          g_config.get("RabbitMQ", "submissionQueue"))
